    ...
    if( !function_exists('get_number_malted_mail_unread'))
    {
        function get_number_malted_mail_unread()
        {
            if (checkLogin())
            {
        		...
        		return $ci->UserModel->num_new_messages($user_info['id']);
            }
        }
    }
...
function checkLogin()
{
	// get the code igniter instance
	$ci =& get_instance();
	// checked if user_info exists.  It will if the user has logged in.
...
function createHeader($logged)
{
	...
	if($logged)
	{
		...
		// get the number of unread private messages
		$num = $ci->UserModel->num_new_messages($user_info['id']);
		...
		$str .= '
		<div id="loginInfo">
			<p>' . $pms . 'Welcome, <a href="' . base_url() . 'user/profile/' . $user_info['id'] . '">' . $user_info['username'] . '</a> <span style="font-size: 74%;">(if you are finished, <a href="' . base_url() . 'user/logout">logout</a>)</span></p>
		</div>
		';
	}
	else

...
class User extends CI_Controller
{
	...
	public function login()
	{
		...
		$logged = $this->_data['logged'] = checkLogin();
		...
		else
		{
			...
			{ 
				...
				if (!$boolean)
				{
					...
					$this->_data['error'] = '<div class="formError">The email address and/or password you provided are incorrect.</div>';
				...
				else
				{
					...
					// convert the URI, if it exists, to a useable URL.  In MY_url_helper.php
					$args = swap_out_uri(array('_', '.', '-'), '/', $uri);					
					// go to the profile if not redirected
					$url = 'user/profile';
					if (!empty($args))
					{
						$url = $args;
	...
	private function profileRightLinks($config = array()) {    //echo '<pre>'; print_r($config); exit;     
    ...
    public function beer() {
        ...
        $this->load->library('userslib');
        // get the user id
        $user_id = $this->UserModel->getIDByUsername($user_name);
        //echo '<pre>'; print_r($user_id); exit;
        // proceed to get information for display
        $array_output = $this->userslib->display_user_beers($user_name, $user_id, $userInfo['id']);
        // start the output
        $this->_data['leftCol'] = $array_output['left_column'];
        // set the right column output
        $this->_data['rightCol'] = $array_output['right_column'];
        // check if the user is logged in
        if($userInfo !== false) {
            // set the right column output
            $this->_data['rightCol'] .= $this->profileRightLinks(array('userInfo' => $userInfo, 'id' => (empty($user_id) ? 0 : $user_id)));
        }
        // set the SEO data
    ...
    public function style() {
        ...
        $user_id = $this->UserModel->getIDByUsername($user_name);
        // proceed to get information for display
        $array_output = $this->userslib->display_user_styles($user_name, $user_id, $userInfo['id']);
        // start the output
        $this->_data['leftCol'] = $array_output['left_column'];
        // set the right column output
        $this->_data['rightCol'] = $array_output['right_column'];

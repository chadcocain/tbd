...
class Establishment extends CI_Controller {
    ...
    public function ajax_google_map_lat_lng()
    {
        ...
        if (!empty($establishment_id) && $latitude != FALSE && $longitude != FALSE)
        {
            $this->load->model('EstablishmentModel', '', TRUE);
    ...
    public function google_map_processor()
    {
        ...
        if ($establishment_id !== FALSE && $address !== FALSE)
        {
	...
	public function googleMaps() {
		...
		if($establishmentID == false) {
            ...
            //echo '<pre>'; print_r($closeBy);
            // holder for the right column
            $str = '';
            // check for results
            if(!empty($closeBy) && is_array($closeBy)) {
            //if($closeBy != false) {
                ...
                foreach($closeBy as $store) {
                    ...
                    $str .= '
                        <li>
                            <p><a href="' . base_url() . 'brewery/info/' . $store['establishmentID'] . '">' . $store['name'] . '</a> - <span class="bold">' . $distance . '</span> ' . $miles . '</p>
                ...
                $str .= '</ul>';
                $array_js['close_by'] = $closeBy;
            } else {
                $str = '<h4><span>Establishments Within ' . RADIUS_SEARCH . ' Miles</span></h4><ul><li>No results</li></ul>';
            }
            //array_unshift($array_js['close_by'], $est);
            if(!array_key_exists('close_by', $array_js)) {
                $array_js['close_by'][] = $est;
            } else {
                array_unshift($array_js['close_by'], $est);
            }

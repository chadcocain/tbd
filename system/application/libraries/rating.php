...
class Rating {
	...
	public function getRatingsLimit($limit = 0) {
		// get all the beers
		//$items = $this->ci->RatingModel->getAll($limit);
		$items = $this->ci->RatingModel->getAll(BEER_REVIEW_HOME);
		...
		/*foreach($items as $item) {
			$alcoholContent = empty($item['alcoholContent']) ? '&nbsp;' : $item['alcoholContent'] . '%';
			$seasonal = empty($item['seasonal']) ? 'No' : 'Yes';
			$img = checkForImage(array('picture' => $item['picture'], 'id' => $item['beerID']));
			$str .= '			
			...
			$str .= '
						...
						<dt>Date Tasted:</dt><dd>' . $item['dateTasted'] . ' by ' . $item['username'] . '</dd>
						<dt>Date Reviewed:</dt><dd>' . $item['dateAdded'] . '</dd>
						<dt>Rating:</dt><dd>' . $item['rating'] . '</dd>
						<dt>Color:</dt><dd>' . $item['color'] . '</dd>
						<dt>Packaging:</dt><dd>' . $item['package'] . '</dd>
						<dt>Price:</dt><dd>$' . $item['price'] . '</dd>

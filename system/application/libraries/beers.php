	...
	public function showBeerRatings($id, $ajax = false, $logged = false) {
		...
		if($ajax === false) {
			...
			if($userInfo != false) {
				...
				$array = array(
					'uri' => substr($this->ci->uri->uri_string(), 1)
					, 'search' => array('/')
					, 'replace' => '_'
				);
			...
			$retired = '';
			// check if the beer is retired
			if($beers[0]['retired'] == '1') {
							...
							<div id="beerInfo">
								...
								<div class="establishmentLocation bold">
									<p>
										...
										<a style="margin-left: 10px;" href="' . $beers[0]['url'] . '" target="_blank"><img src="' . base_url() . 'images/web.jpg" title="' . $beers[0]['name'] . ' web site" alt="' . $beers[0]['name'] . ' web site" /></a>
										' . $breweryHop . '
								...
								<p class="marginTop_4"><span class="bold">ABV:</span> ' . $beers[0]['alcoholContent'] . '%</p>
			';

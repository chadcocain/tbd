...
class RatingModel extends CI_Model {
    ...
    public function getRatingsByUserIDForUserProfile($id, $offset) {
        // temporary holder for results
        $array = '';
        ...
        $query = '
            SELECT
                COUNT(r.id) AS total_beers
            FROM ratings r
            INNER JOIN beers be ON be.id = r.beerID
            INNER JOIN establishment e ON e.id = be.establishmentID
            INNER JOIN styles st ON be.styleID = st.id
            WHERE
                r.userID = ' . mysqli_real_escape_string($this->db->conn_id, $id) . '
                AND r.active = "1"
        ';        
        // get the record set
        $rs = $this->db->query($query);
        ...
        if($total > 0) {                
            $query = '
                ...
                ORDER BY 
                    be.beerName ASC
                LIMIT ' . $offset . ', ' . USER_BEER_REVIEWS_PAGINATION
            ;
